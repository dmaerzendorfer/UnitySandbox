...
{
    //based on https://www.youtube.com/watch?v=VwGiwDLQ40A
    ...
    public class MeshDestroy : MonoBehaviour
    {
        ...
        [Tooltip("How often the mesh should be cut")]
        public int CutCascades = 1;

        [Tooltip("The amount of outwards force that should be applied to parts")]
        public float ExplodeForce = 0;

        [Tooltip("A fail save that should destroy parts that are too small eG have been cut x times already")]
        public int MaxDestroyLevel = 3;

        public bool animatePartsOut = true;

        [EnableIf("animatePartsOut")]
        public float disappearTime = 2f;

        [EnableIf("animatePartsOut")]
        public float waitTimeBeforeDisappear = 2f;
        ...
        public class PartMesh
        {
            ...
            public GameObject MakeGameobject(MeshDestroy original)
            {
                ...
                meshDestroy.animatePartsOut = original.animatePartsOut;
                meshDestroy.waitTimeBeforeDisappear = original.waitTimeBeforeDisappear;
                meshDestroy.disappearTime = original.disappearTime;

                if (meshDestroy.MaxDestroyLevel <= 0)
                {
                    meshDestroy.enabled = false;
                    Destroy(meshDestroy);
                    if (meshDestroy.animatePartsOut)
                        DOTween.Sequence().SetDelay(meshDestroy.waitTimeBeforeDisappear + Random.Range(0f, 0.1f))
                            .Append(
                                GameObject.transform.DOScale(new Vector3(0.1f, 0.1f, 0.1f), meshDestroy.disappearTime)
                                    .SetEase(Ease.InExpo)
