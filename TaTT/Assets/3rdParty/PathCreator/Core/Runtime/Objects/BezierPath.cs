...
namespace PathCreation
{
    ...
    public class BezierPath
    {
        ...
        public BezierPath(Vector3 centre, bool isClosed = false, PathSpace space = PathSpace.xyz)
        {
            ...
            perAnchorWeight = new List<float>() { 1, 1 };
        ...
        public BezierPath(IEnumerable<Vector3> points, bool isClosed = false, PathSpace space = PathSpace.xyz)
        {
            ...
            else
            {
                ...
                perAnchorWeight = new List<float>(new float[] { 1, 1 });
        ...
        public void AddSegmentToEnd(Vector3 anchorPos)
        {
            ...
            perAnchorWeight.Add(perAnchorWeight[perAnchorWeight.Count - 1]);
        ...
        public void AddSegmentToStart(Vector3 anchorPos)
        {
            ...
            perAnchorWeight.Insert(0, perAnchorWeight[0]);
        ...
        public void SplitSegment(Vector3 anchorPos, int segmentIndex, float splitTime)
        {
            ...
            float weightPrev = perAnchorWeight[segmentIndex];
            float weightNext = perAnchorWeight[newAnchorAngleIndex];
            float splitWeight = Mathf.LerpAngle(weightPrev, weightNext, splitTime);
            perAnchorWeight.Insert(newAnchorAngleIndex, splitWeight);
        ...
        public void DeleteSegment(int anchorIndex)
        {
            ...
            if (NumSegments > 2 || !isClosed && NumSegments > 1)
            {
                ...
                perAnchorWeight.RemoveAt(anchorIndex / 3);
        ...
        /// Returns an array of the 2 weights making up the segment (anchor1, anchor2)
        public float[] GetWeightsInSegment(int segmentIndex)
        {
            segmentIndex = Mathf.Clamp(segmentIndex, 0, NumSegments - 1);
            //calc anchorIdx
            int firstAnchorIdx = segmentIndex;
            int secondAnchorIdx = firstAnchorIdx + 1;
            if (secondAnchorIdx >= perAnchorWeight.Count) secondAnchorIdx = 0;

            return new float[]
            {
                GetAnchorWeight(firstAnchorIdx),
                GetAnchorWeight(secondAnchorIdx)
            };
        }
        ...
        /// Get the weight at a particular anchor
        public float GetAnchorWeight(int anchorIndex)
        {
            //dirty fix since old paths didnt have a weight yet... //todo ??? why does it still happen?
            if (anchorIndex > perAnchorWeight.Count - 1) return 1;
            return perAnchorWeight[anchorIndex];
        }

        /// Set the desired weight at a particular anchor 
        public void SetAnchorWeight(int anchorIndex, float weight)
        {
            //can happen if trying to set weight on an old path that didnt have weights before
            if (anchorIndex > perAnchorWeight.Count - 1)
            {
                perAnchorWeight.Resize(anchorIndex + 1, 1);
            }

            if (perAnchorWeight[anchorIndex] != weight)
            {
                perAnchorWeight[anchorIndex] = weight;
                NotifyPathModified();
            }
        }

        /// Sets all weights to the given value
        public void SetAllWeights(float value)
        {
            for (int i = 0; i < perAnchorWeight.Count; i++)
            {
                perAnchorWeight[i] = value;
            }
