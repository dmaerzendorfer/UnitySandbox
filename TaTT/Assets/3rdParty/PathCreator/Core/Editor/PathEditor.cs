    ...
    public class PathEditor : Editor
    {
        ...
        void DrawBezierPathInspector()
        {
            using (var check = new EditorGUI.ChangeCheckScope())
            {
                ...
                if (data.showPathOptions)
                {
                    ...
                    if (handleIndexToDisplayAsTransform != -1)
                    {
                        ...
                        using (new EditorGUI.IndentLevelScope())
                        {
                            ...
                            //draw the setting for the weight of a point
                            if (handleIndexToDisplayAsTransform % 3 == 0)
                            {
                                var anchorIndex = handleIndexToDisplayAsTransform / 3;
                                var currentWeight = creator.bezierPath.GetAnchorWeight(anchorIndex);
                                var newWeight = EditorGUILayout.FloatField("Weight", currentWeight);
                                if (newWeight != currentWeight)
                                {
                                    Undo.RecordObject(creator, "Set Weight");
                                    creator.bezierPath.SetAnchorWeight(anchorIndex, newWeight);
                                }
                            }
                        }
                    }
                ...
                data.showWeights = EditorGUILayout.Foldout(data.showWeights, new GUIContent("Weights Options"), true,
                    boldFoldoutStyle);
                if (data.showWeights)
                {
                    var newGlobalWeight = EditorGUILayout.FloatField("Global Weight", data.globalWeight);
                    if (newGlobalWeight != data.globalWeight)
                        data.globalWeight = newGlobalWeight;
                    if (GUILayout.Button("Apply global Weight"))
                    {
                        Undo.RecordObject(creator, "Set all Weights");
                        bezierPath.SetAllWeights(data.globalWeight);
                    }

                    if (GUILayout.Button("Reset all Weights to 1"))
                    {
        ...
        void DrawVertexPathInspector()
        {
            ...
            if (data.showVertexPathDisplayOptions)
            {
                using (var check = new EditorGUI.ChangeCheckScope())
                {
                    ...
                    data.showWeightsInVertexMode =
                        GUILayout.Toggle(data.showWeightsInVertexMode, new GUIContent("Show Weights"));
        ...
        void DrawVertexPathSceneEditor()
        {
            ...
            if (data.showWeightsInVertexMode)
            {
                Handles.color = globalDisplaySettings.weights;
                Vector3[] weightLines = new Vector3[creator.path.NumPoints * 2];
                for (int i = 0; i < creator.path.NumPoints; i++)
                {
                    weightLines[i * 2] = creator.path.GetPoint(i);
                    weightLines[i * 2 + 1] = creator.path.GetPoint(i) +
                                             Vector3.up * creator.path.localWeights[i];
                }

                Handles.DrawLines(weightLines);
            }
        }
