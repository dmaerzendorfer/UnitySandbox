...
namespace PathCreation
{
    ...
    public class VertexPath
    {
        ...
        VertexPath(BezierPath bezierPath, VertexPathUtility.PathSplitData pathSplitData, Transform transform)
        {
            ...
            localWeights = new float[numVerts];
            ...
            for (int i = 0; i < localPoints.Length; i++)
            {
                ...
                localWeights[i] = pathSplitData.weights[i];
        ...
        /// Gets weight on path based on distance travelled.
        public float GetWeightAtDistance(float dst,
            EndOfPathInstruction endOfPathInstruction = EndOfPathInstruction.Loop)
        {
            float t = dst / length;
        ...
        /// Gets weight on path based on 'time' (where 0 is start, and 1 is end of path).
        public float GetWeightAtTime(float t, EndOfPathInstruction endOfPathInstruction = EndOfPathInstruction.Loop)
        {
            var data = CalculatePercentOnPathData(t, endOfPathInstruction);
            return Mathf.Lerp(GetWeight(data.previousIndex), GetWeight(data.nextIndex), data.percentBetweenIndices);
        }
        ...
        /// Gets the weight of the point on the path
        public float GetWeight(float t, EndOfPathInstruction endOfPathInstruction = EndOfPathInstruction.Loop)
        {
            var data = CalculatePercentOnPathData(t, endOfPathInstruction);
            return Mathf.Lerp(localWeights[data.previousIndex], localWeights[data.nextIndex],
                data.percentBetweenIndices);
        }
